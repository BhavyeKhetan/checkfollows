...
import { ADDITIONAL_ACCOUNT_ITEM_KIND } from "@/lib/account-capacity-rules";
...
export async function POST(request: Request) {
  try {
    ...
    if (action === "change_plan") {
      ...
      if (additionalAccountItem && (additionalAccountItem.quantity || 0) > 0) {
        items.push({
          id: additionalAccountItem.id,
          price: getAdditionalAccountPriceId(cadence),
