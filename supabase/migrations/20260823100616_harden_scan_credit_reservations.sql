-- A repeated idempotency key must never authorize a second provider scan.
-- Return the existing reservation id with reserved=false so the API can report
-- an in-progress/already-processed request without spending or scanning again.
